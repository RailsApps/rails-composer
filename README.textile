...
An application template that creates a starter application for Rails.
...
Choices of starter applications will differ depending on whether you are using Rails 4.1 or Rails 3.2.
...
Enter "1" to select *Build a starter application*. You'll see a warning:

<pre>
railsapps  Please upgrade to Rails 4.1 to get the starter apps.
</pre>

Then you can continue with the "Build Your Own Application" options (see below).
...
None of these Rails 3.2 example applications are available for newer versions of Rails.

h4. Options

The application generator template may ask you for additional preferences:
...
You'll have a choice of databases. If you choose something other than SQLite, the database server must be running.
